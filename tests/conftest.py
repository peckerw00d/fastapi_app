...
async def run_migrations():
    ...
    os.system("alembic update heads")
...
@pytest.fixture
async def get_user_for_database(asyncpg_pool):

    async def get_user_for_database_by_uuid(user_id: str):
        async with asyncpg_pool.acquire() as connection:
            return await connection.fetch("""SELECT * FROM users WHERE user_id = $1;""", user_id)
        
    return get_user_for_database_by_uuid
